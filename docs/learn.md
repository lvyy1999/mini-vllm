...
本文档基于仓库当前代码，按“算子 -> 模型 -> KV Cache -> 调度 -> 执行引擎 -> Benchmark”的顺序介绍一个轻量级 LLM 推理引擎。项目面向推理而不是训练，当前以 Qwen3-0.6B 为主要实测模型，并包含尚未完整实测的 Llama-3.2-1B-Instruct 适配；引擎实现了张量并行、Paged KV Cache、前缀缓存、chunked prefill、可选 INT8 KV Cache、Triton Attention 和 decode CUDA Graph。

> 核心代码位于 `src/minivllm`。当前 `ModelRunner` 无论单卡还是多卡都会初始化 NCCL，因此运行环境需要 CUDA、NCCL，以及支持 CUDA 的 PyTorch。
...
这种接口让 DecoderLayer 可以沿层传递 residual，减少重复的残差处理代码。`weight` 是 `nn.Parameter`，可从 Hugging Face checkpoint 加载。
...
- Llama 3.2：`is_llama3=True`，根据 factor、high/low frequency factor 和原始上下文长度调整频率。
...
| Llama-3.2-1B-Instruct | `LlamaForCausalLM` | 无 Q/K Norm、Llama 3.2 RoPE scaling、可配置 MLP bias |
...
只有完整且已经完成计算的 block 才会进入前缀缓存。最后一个未满 block 不复用，因为后续 decode 仍可能继续写入。
...
加载器支持本地目录和 Hugging Face 模型名，只读取 safetensors/json 文件。权重先在 CPU 侧读取，再由参数自身的 `weight_loader` 拷贝或切分到已经位于 GPU 的模型参数。
...
```python
[1, 2, 4, 8] + list(range(16, max_graph_bs + 1, 16))
```

如果 `max_graph_bs` 不是 16 的整数倍，代码会把它额外追加到 `self.graph_bs`，保证所有满足 `bs <= max_graph_bs` 的实际 batch 都能找到可用图。例如 `max_graph_bs=100` 时，列表末尾会包含 `96, 100`。
...
CUDA Graph 的 hidden-state 输出 buffer 显式使用 `self.model_dtype`，因此能够和 FP16/BF16 模型保持一致。`enforce_eager=True` 会完全绕过 CUDA Graph 路径。
...
`LLM` 当前是 `LLMEngine` 的轻量别名。初始化顺序为：
...
文件：[benchmark_prefilling.py](../benchmark_prefilling.py)
...
文件：[benchmark_decoding.py](../benchmark_decoding.py)
...
文件：[benchmark_tps.py](../benchmark_tps.py)
...
`--respect-eos` 会允许各后端遇到 EOS 提前停止；默认忽略 EOS，更适合固定输出长度的吞吐量比较。详细 benchmark 结果见项目根目录 [README.md](../README.md)。
...
7. `model_runner.py`：串联输入准备、多卡执行、不同 dtype 的缓存分配、显存估算和 CUDA Graph。
8. `llm_engine.py`：理解对外 API 与完整生成循环。
9. 三个 benchmark：分别观察 prefill、decode 算子性能和端到端吞吐量。
...
8. 优化 TP 通信与显存占用，评估自定义 all-reduce、通信计算重叠和不同 `world_size` 下的吞吐量。
