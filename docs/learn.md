...
> 核心代码位于 `src/minivllm`。当前 `ModelRunner` 无论单卡还是多卡都会初始化 NCCL，因此运行环境需要 CUDA、NCCL，以及支持 CUDA 的 PyTorch。张量并行核心层与 NCCL 通信已通过真实双 GPU 正确性测试，Qwen3-0.6B 也已完成 eager 模式双卡端到端生成；多卡 CUDA Graph、INT8 KV Cache 和吞吐性能仍未验证。
...
这套多进程与 collective 路径已通过真实双 GPU 正确性测试，并使用 Qwen3-0.6B、`world_size=2`、`enforce_eager=True` 完成端到端生成。该验证尚不覆盖多卡 CUDA Graph、INT8 KV Cache、Prefix Cache 或 Chunked Prefill 场景。
...
当前测试套件共 161 项，已在真实双 GPU CUDA/NCCL 环境中全部通过。`test_attention.py` 会实际运行 INT8 KV Cache 写入、decode 和 chunked prefill Triton kernel；`test_tensor_parallel.py` 会启动两个 NCCL rank，验证 TP Linear、Embedding、LM Head、MLP 和 Qwen3 Prefill Attention 的数值正确性。
...
8. 为双卡 TP 增加吞吐测试，并继续验证多卡 CUDA Graph、INT8 KV Cache 和复杂调度场景，再评估自定义 all-reduce、通信计算重叠及不同 `world_size` 下的性能。
