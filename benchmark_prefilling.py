...
def report_correctness(name: str, reference: torch.Tensor, candidate: torch.Tensor, atol=2e-2, rtol=2e-2):
    ...
    print(f"      Correctness vs PyTorch [{name}]: {status}, max_abs_err={max_abs_err:.6f}")
    return is_close
...
def analyze_kernel_programs():
    """Compare Triton program counts inside one kernel launch."""

    print("\n" + "="*80)
    print("TRITON PROGRAM GRID ANALYSIS")
    print("="*80)
    ...
    naive_programs = num_seqs * num_heads

    # Flash Attention
    num_blocks_m = (seq_len + BLOCK_M - 1) // BLOCK_M
    flash_grid = (num_blocks_m, num_heads, num_seqs)
    flash_programs = num_blocks_m * num_heads * num_seqs

    print(f"\nFor {num_seqs} sequences × {seq_len} tokens:")
    print(f"  Naive Triton grid:    {naive_grid}")
    print(f"  Naive total programs: {naive_programs}")
    print(f"\n  Flash Attention grid: {flash_grid}")
    print(f"  Flash total programs: {flash_programs}")
    print(f"\n  Ratio: Flash uses {flash_programs/naive_programs:.1f}x more Triton programs")
    print("\n  Note: these are programs within a single Triton kernel launch, not separate launches.")
...
def benchmark(num_seqs, seq_len, num_heads=32, num_kv_heads=8, head_dim=128, num_iter=50):
    ...
    if seq_len <= max_safe_seq:
        ...
        print(f"      {t*1000:.3f} ms")
        report_correctness("Naive Triton", outputs['pytorch'], outputs['naive'])
    else:
    ...
    print(f"      {t*1000:.3f} ms")
    report_correctness("Flash Attention", outputs['pytorch'], outputs['flash'])
