...
from minivllm.models.model_factory import create_model
...
class ModelRunner:
    ...
    def warmup_model(self):
        ...
        seqs = [Sequence([0] * seq_len) for _ in range(batch_size)]
        for seq in seqs:
    ...
    def allocate_kv_cache(self):
        ...
        assert (
            num_available_kv_blocks > 0
        ), f"Not enough memory to hold at least one block of KV cache on rank {self.rank}"

        # Synchronize max_cached_blocks across all ranks if needed, each rank computed
        # num_available_kv_blocks from its own free GPU memory independently.
        if self.world_size > 1:
            print(
        ...
        if self.rank == 0:
            print(
                f"[Rank 0] Global max_cached_blocks (min): {self.config.max_cache_blocks}"
            )
        ...
        # use zeros() instead of empty() to avoid garbage values
    ...
    def prepare_block_tables(cls, seqs: list[Sequence]):
        ...
        block_tables = [
            seq.block_table + [-1] * (max_num_blocks - len(seq.block_table))
            for seq in seqs
        ]  # padding with -1
    ...
    def prepare_prefill(self, seqs: list[Sequence]) -> tuple[Tensor, Tensor]:
        ...
        positions = torch.tensor(positions, dtype=torch.long, pin_memory=True).cuda(
            non_blocking=True
        )
    ...
    def run_model(
        ...
        if is_prefill or self.enforce_eager or input_ids.size(0) > self.max_graph_bs:
            ...
            graph_bs = next(bs_ for bs_ in self.graph_bs if bs_ >= bs)
    ...
    def capture_cudagraph(self) -> None:
        ...
        input_ids = torch.zeros(
            self.max_graph_bs, dtype=torch.long, device=f"cuda:{self.rank}"
        )
        positions = torch.zeros(
            self.max_graph_bs, dtype=torch.long, device=f"cuda:{self.rank}"
        )
        slot_mapping = torch.full(
            (self.max_graph_bs,), -1, dtype=torch.int32, device=f"cuda:{self.rank}"
        )
        context_lens = torch.zeros(
            self.max_graph_bs, dtype=torch.int32, device=f"cuda:{self.rank}"
        )
